...
export async function runTestSuite({ includeRuntime = true, silent = false } = {}) {
  ...
  checkFileExists(results, "AI coding tools/scripts/agent-dev-workflow.mjs");
  ...
  if (fs.existsSync(path.join(root, "AI coding tools/scripts/agent-dev-workflow.mjs"))) {
    const workflow = readFile("AI coding tools/scripts/agent-dev-workflow.mjs");
    record(results, {
      status:
